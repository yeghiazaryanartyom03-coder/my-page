...
export function MatchHomePage({ match }: MatchProps) {
  return (
    <NavLink
      to="/"
      className="match-conteiner w-70 h-77 text-white flex flex-col justify-end items-center rounded-3xl gap-y-10 bg-[rgb(9,15,45)] no-underline z-1020 "
    >
      <div className="prevew flex flex-row items-center gap-7.5 transition-transform duration-500 ease-in-out">
        <div className="left-team flex flex-col items-center">
          <img
            src={match.isInHome ? BarcaIcon : match.image}
            className="match-logo w-12 h-12"
          />
          <div className="team-name">
            {match.isInHome ? "Barcelona" : match.opponent}
          </div>
        </div>
        ...
        <div className="right-team flex flex-col items-center">
          <img
            src={match.isInHome ? match.image : BarcaIcon}
            alt=""
            className="match-logo w-12 h-12"
          />
          <div className="team-name">
            {match.isInHome ? match.opponent : "Barcelona"}
          </div>
        </div>
      </div>
